...
import type { MaterialItem, Photo, ServiceReport, TimeEntry, WorkDay } from './types';
...
/** Liest den kompletten Datenbestand (alle Berichte + Zeiten/Material/Fotos/Tagesstempel) für eine Sicherung. */
export async function getAllData(): Promise<AllData> {
  const db = await getDB();
  const [reports, timeEntries, materialItems, photos, workDays] = await Promise.all([
    db.getAll('reports'),
  ...
  return { reports, timeEntries, materialItems, photos, workDays };
}
 ...
 * neue IDs werden ergänzt. Läuft in einer Transaktion über alle fünf Stores.
 */
export async function restoreAllData(data: AllData): Promise<void> {
  const db = await getDB();
  const tx = db.transaction(['reports', 'timeEntries', 'materialItems', 'photos', 'workDays'], 'readwrite');
